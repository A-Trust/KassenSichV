...
Pin States:
    	...
    	adminPinTransportState				= 0x01
    	adminPukTransportState				= 0x02
    	timeAdminPinTransportState			= 0x04
    	timeAdminPukTransportState			= 0x08
 */
typedef int32_t PinStateFlags;

/*
Sync Variants:
    noInput                             = 0x00
    utcTime                             = 0x01
    generalizedTime                     = 0x02
    unixTime                            = 0x03
 */
typedef int32_t SyncVariants;
...
/*
 Get TSE PIN object status.
 @param[out] pinState
                PIN state flags result buffer [REQUIRED].
...
/*
 Get Wear Indicator
 @param[out] wearIndicator
               For values below 100, typical data retention is more than 10 years. Bigger values indicate shorter data retention, but at least 1 year.
 */
int32_t cv_getWearIndicator(uint32_t *wear_indicator);

/*
see @ref getWearIndicator
 */
...
const uint8_t *cv_getUniqueIdWithTse(uint32_t *len, const int8_t *tse_id, uint32_t tse_len);

const int8_t *cv_getApiVersionString(void);

const int8_t *cv_getFirmwareId(void);

const int8_t *cv_getFirmwareIdWithTse(const int8_t *tse_id, uint32_t tse_len);

const int8_t *cv_getImplementationVersionString(void);

/*
 Supplies size of available log memory in bytes
 @param[out] sizeOfMemory
                size of available log memory in bytes [REQUIRED]
...
/*
 Supplies the proposed update interval for the CSP time base (number of seconds)
 @param[out] timeSyncInterval
                 sync interval in seconds [REQUIRED]
...
/*
 Supplies flags for the supported time formats of the TSE.
 @param[out] supportedSyncVariant
                Buffer for the supported time sync format [REQUIRED]
 @return if the execution of the function has been successful, the return value ExecutionOk is returned.
 */
int32_t cv_getTimeSyncVariant(SyncVariants *variant);

/*
see @ref getTimeSyncVariant
 */
...
/*
 Read all mappings of ERS and belonging public key serial number.
 @param[out] mappingData
                Buffer for the returned DER encoded mapping data (sequence of ERS mappings) [REQUIRED]
                If successfully executed, the buffer has to freed by the function caller using the function @ref asigntse_free.
 @param[out] mappingDataLength
                length of DER encoded mapping data [REQUIRED]
 @return if the execution of the function has been successful, the return value EXECUTION_OK is returned.
 */
...
/*
 ...
 This assigns an existing private key to a client (cash register).
 @param[in] clientId
                serial number of the ERS [REQUIRED]
 @param[in] clientIdLength
                length of the array that represents the clientId  [REQUIRED]
 @param[in] serialNumberKey
                ID of the key to be mapped (SHA256 hash value of the public key)
 @param[in] serialNumberLength
                length of the array that represents the serialNumberKey  [REQUIRED]

 @return @ref EXECUTION_OK                                 execution of the function has been successful
 @return @ref ERROR_SIGNING_SYSTEM_OPERATION_DATA_FAILED   determination of the log message parts for the system operation data by the Secure Element failed
 @return @ref ERROR_STORING_INIT_DATA_FAILED               storing of the data for the description of the SE API failed
 @return @ref ERROR_RETRIEVE_LOG_MESSAGE_FAILED            execution of the Secure Element functionality to retrieve log message parts has failed
 @return @ref ERROR_STORAGE_FAILURE                        storing of the log message has failed
 @return @ref ERROR_CERTIFICATE_EXPIRED                    the certificate with the public key for the verification of the appropriate type of log messages is expired.
                                                           Even if a certificate expired, the log message parts are created by the Secure Element and stored by the SE API.
 @return @ref ERROR_SECURE_ELEMENT_DISABLED               the Secure Element has been disabled
 @return @ref ERROR_USER_NOT_AUTHORIZED                   the user who has invoked the function initializeDescriptionNotSet is not authorized to execute this function
 @return @ref ERROR_USER_NOT_AUTHENTICATED                the user who has invoked the function initializeDescriptionNotSet has not the status authenticated
 @return @ref ERROR_NO_SUCH_KEY                           unknown serialNumberKey
 @return @ref ERROR_ERS_ALREADY_MAPPED                    the serial number of an ERS is already mapped to a signature key
 */
...
/*
 Initialize TSE pin objects.
 @param[in] adminPin
                Admin PIN value or NULL
 @param[in] adminPinLength
                length of Admin PIN value: 8 (current Admin PIN length) or 0
 @param[in] adminPuk
                Admin PUK value or NULL (PUK not touched)
 @param[in] adminPukLength
                length of Admin PUK value: 10 (current Admin PUK length) or 0
 @param[in] timeAdminPin
                Admin PIN value or NULL
 @param[in] timeAdminPinLength
                length of Admin PIN value: 8 (current Time Admin PIN length) or 0
 @param[in] timeAdminPuk
                Admin PUK value or NULL
 @param[in] timeAdminPukLength
                length of Admin PUK value: 10 (current Time Admin PUK length) or 0

 @return @ref EXECUTION_OK                     execution of the function has been successful
 @return @ref ERROR_TSE_COMMAND_DATA_INVALID   invalid TSE command data
 @return @ref ERROR_SE_COMMUNICATION_FAILED    Secure Element communication failed
 */
...
/*
 Supplies size of log memory in bytes
 @param[out] sizeOfMemory
                size of log memory in bytes [REQUIRED]
...
/*
Registers a new client ID to the transaction signing log key
 */
int32_t cv_registerClientId(const int8_t *client_id, uint32_t client_id_length);

/*
see @ref cv_registerClientId
 */
